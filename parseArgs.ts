...
const parseArgs = (args: string[]): Meters | Exercise => {
  ...
  if (!isNaN(Number(args[2])) && !isNaN(Number(args[3]))) {
    ...
    const dailyExercises: number[] = JSON.parse(args[2]) as number[];
    if (dailyExercises.length < 1)
      throw new Error('There must be atleast one daily exercise done');
    if (!dailyExercises.every((e) => typeof e === 'number'))
